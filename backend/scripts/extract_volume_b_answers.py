...
VOLUME_B_DIR = PROJECT_ROOT / "data" / "volume_b_docs"
SURVEY_CSV = PROJECT_ROOT / "data" / "surveys" / "michlou_survey_tri.csv"
OUTPUT_CSV = PROJECT_ROOT / "data" / "surveys" / "volume_b_answer_distributions.csv"
...
class VolumeBParser:
    ...
    def detect_format(self, rows: list[tuple]) -> dict:
        ...
        for ri in range(sub_header_row + 1, min(sub_header_row + 10, len(rows))):
            ...
            second_cell = _normalize(rows[ri][1]) if len(rows[ri]) > 1 and rows[ri][1] else ""
        ...
        if base_row < 0:
            base_row = fallback_base_row if fallback_base_row >= 0 else sub_header_row + 1
    ...
    def discover_columns(
        self, cat_row: tuple, sub_row: tuple, base_row: tuple
    ) -> dict:
        ...
        for ci in range(len(sub_row)):
            ...
            if "appartenir" in cat_val or "belonging" in cat_val:
                ...
                if label is None and ("moyenne" in sub_val or "middle class" in sub_val):
                    label = "middle"
                if label:
                    demographics.append(
        ...
        return {"total_col": total_col, "demographics": demographics, "base_row_data": base_row}
    ...
    def parse_answer_rows(
        self, rows: list[tuple], format_info: dict
    ) -> list[dict]:
    ...
    def extract_sheet(
        ...
        for answer in answers:
            ...
            for slc in slices:
                ci = slc["col_idx"]
                count_val = self._safe_float(count_row[ci] if ci < len(count_row) else None)
...
def main():
    ...
    logger.success(f"Saved {len(df)} rows to {OUTPUT_CSV}")
